...
import type { ProtectionFundApplication } from "../../web/src/protection-fund-application.js";
import type { ProtectionFundArtifact } from "../../web/src/protection-fund-artifact.js";
import { protectionFundArtifact } from "../../web/src/protection-fund-artifact.js";
import { protectionFundArtifactToA2UI } from "./protection-fund-a2ui.js";
import { conventionalProtectionFundRoute } from "../../web/src/presentation.js";
...
export function createProtectionFundWebAgentAdapter(options: { readonly application: ProtectionFundApplication; readonly principal: CommandPrincipal; readonly createSurfaceId: (artifactId: string) => string }) { return Object.freeze({ get() { const artifact: ProtectionFundArtifact = protectionFundArtifact(options.application, options.principal); const surfaceId=options.createSurfaceId(artifact.id); return Object.freeze({channel:"web-agent" as const,artifact,surfaceId,a2uiMessages:protectionFundArtifactToA2UI({artifact,surfaceId}),fallback:Object.freeze({message:"Guest Protection Fund",conventionalRoute:conventionalProtectionFundRoute()})}); } }); }
