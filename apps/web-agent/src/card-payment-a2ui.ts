...
export function cardPaymentArtifactToA2UI({ artifact, surfaceId }: { readonly artifact: CardPaymentArtifact; readonly surfaceId: string }): readonly A2UIServerMessage[] {
  ...
  const components: A2UIComponent[] = [
    ...
    { id: "card-payment-amount", component: "Text", text: facts.status === "confirmed" ? `Paid: ${amount(facts.amountPaidKobo ?? 0, facts.currency)}` : `Amount due: ${amount(facts.amountDueNowKobo, facts.currency)}` },
    ...
    ...(artifact.actions.length ? [{ id: "card-payment-initialize", component: "Button" as const, child: "card-payment-initialize-label", variant: "primary" as const, action: { event: { name: CARD_PAYMENT_INITIALIZE_CHECKOUT_EVENT, context: { artifactId: artifact.id, offerId: facts.offerId, expectedStatus: "ready", projectionVersion: artifact.projectionVersion } } }, accessibility: { label: "Start secure card checkout" } }, { id: "card-payment-initialize-label", component: "Text" as const, text: "Start secure checkout" }] : []),
  ];
