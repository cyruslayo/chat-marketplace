...
export function cardPaymentArtifactToA2UI({ artifact, surfaceId }: { readonly artifact: CardPaymentArtifact; readonly surfaceId: string }): readonly A2UIServerMessage[] {
  ...
  const components: A2UIComponent[] = [
    { id: "card-payment-root", component: "Column", children: ["card-payment-title", "card-payment-status", "card-payment-unit", "card-payment-dates", "card-payment-amount", "card-payment-deadline", "card-payment-content", "card-payment-actions"] },
    ...
    ...(artifact.actions.length ? [{ id: "card-payment-initialize", component: "Button" as const, child: "card-payment-initialize-label", variant: "primary" as const, action: { event: { name: CARD_PAYMENT_INITIALIZE_CHECKOUT_EVENT, context: { artifactId: artifact.id, offerId: facts.offerId, expectedStatus: artifact.actions[0]?.expectedStatus ?? facts.status, expectedPurpose: artifact.actions[0]?.expectedPurpose ?? "stay", expectedJourneyVersion: facts.journeyVersion ?? 1, expectedStage: facts.journeyStage ?? facts.status, depositPolicyVersion: facts.depositPolicyVersion ?? "", projectionVersion: artifact.projectionVersion } } }, accessibility: { label: facts.status === "deposit_required" ? "Continue to refundable deposit" : "Start secure card checkout" } }, { id: "card-payment-initialize-label", component: "Text" as const, text: "Start secure checkout" }] : []),
  ];
