...
export interface MidStayIncidentRepository { findByReservationId(reservationId: string): MidStayIncidentRecord | null; findByIncidentId(incidentId: string): MidStayIncidentRecord | null; createIfAbsent(record: MidStayIncidentRecord): MidStayIncidentRecord; update(expectedIncidentVersion: number, mutation: (current: MidStayIncidentRecord) => MidStayIncidentRecord): MidStayIncidentRecord; }
export class InMemoryMidStayIncidentRepository implements MidStayIncidentRepository {
  ...
  createIfAbsent(record: MidStayIncidentRecord) { const existing = this.#records.get(record.incidentId); if (existing) return existing; this.#records.set(record.incidentId, Object.freeze({ ...record })); return record; }
  update(expected: number, mutation: (current: MidStayIncidentRecord) => MidStayIncidentRecord) { const current = [...this.#records.values()].find((r) => r.incidentVersion === expected); if (!current) throw new Error("STALE_ACTION"); const next = mutation(current); if (next.incidentVersion !== expected + 1) throw new Error("Invalid incident version"); this.#records.set(next.incidentId, Object.freeze({ ...next })); return next; }
}
