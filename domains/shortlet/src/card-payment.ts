...
export class CardPaymentManager {
  ...
  verifyAndConfirmCardPayment(
    ...
    try {
      ...
      if (session.purpose === "security_deposit") { this.#compensateStay(offerId); this.#compensateDeposit(offerId, now); }
      if (rollbackFailed) this.#markFinalizationReconciliation(offerId);
      throw error;
    }
