...
export interface AuthoritativeReleaseInput {
  ...
  readonly accessVersion: string; readonly accessStatus: "verified_access" | "late_voluntary_arrival"; readonly verifiedAccessAt: string; readonly protectionWindowStartsAt: string; readonly economics: AuthoritativeRevenueEconomics;
...
export interface ProductionRevenueReleaseRecord {
  readonly releaseId: string; readonly releaseVersion: 1; readonly reservationId: string; readonly contractId: string; readonly contractVersion: number; readonly unitId: string; readonly tenantId: string; readonly operatorId: string; readonly accessVersion: string; readonly accessStatus: "verified_access" | "late_voluntary_arrival"; readonly verifiedAccessAt: string; readonly protectionWindowStartsAt: string; readonly protectionWindowEndsAt: string; readonly economicsVersion: string; readonly commissionPolicyVersion: string; readonly commissionRate: 0.08 | 0.1 | 0.12; readonly commissionBaseKobo: number; readonly commissionKobo: number; readonly operatorNetKobo: number; readonly payoutPlan: "fast_payout" | "full_post_stay"; readonly payoutPlanVersion: string; readonly payableNowKobo: number; readonly routineReserveTrancheKobo: number; readonly reserveReviewEligibleAt?: string; readonly deferredPostStayKobo: number; readonly postStayPayableEligibleAt?: string; readonly riskHoldVersion: string; readonly riskHoldKobo: number; readonly ledgerJournalId: string; readonly earnedCommissionRecordId: string; readonly effectiveCheckoutVersion: string; readonly releasedAt: string; readonly currency: "NGN";
}
...
export class RevenueReleaseManager {
   ...
   */
  /** @deprecated Compatibility-only caller-driven API. Production uses RevenueReleaseApplication. */
  ...
  buildProductionRelease(input: AuthoritativeReleaseInput): { release: ProductionRevenueReleaseRecord; journalLines: readonly RevenueLedgerLine[]; earnedCommission: EarnedCommissionRecord } {
    ...
    const start = new Date(input.protectionWindowStartsAt); const end = new Date(start.getTime() + 24 * 60 * 60 * 1000); if (!Number.isFinite(start.getTime()) || !Number.isFinite(end.getTime()) || input.now.getTime() < end.getTime()) throw new Error("Check-In Protection Window active: Revenue release requires Verified Access plus 24 hours");
   ...
   */
  /** @deprecated Compatibility-only helper. Production adjustments use RevenueAccountingRepository. */
