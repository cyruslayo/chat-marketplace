...
const clone = (j: BookingPaymentJourney): BookingPaymentJourney => Object.freeze({ ...j, stay: Object.freeze({ ...j.stay }), deposit: Object.freeze({ ...j.deposit }), compensation: Object.freeze({ ...j.compensation }) });
export class InMemoryBookingPaymentJourneyRepository implements BookingPaymentJourneyRepository {
  ...
  createIfAbsent(input: { offerId: string; paymentMethod: "fresh_card" | "bank_transfer"; originalPaymentDeadline: string; stayAmountKobo: number; deposit: SecurityDepositPolicySnapshot | null }): BookingPaymentJourney {
    ...
    const d = input.deposit; const journey = clone({ offerId: input.offerId, journeyVersion: 1, paymentMethod: input.paymentMethod, originalPaymentDeadline: input.originalPaymentDeadline, requiredDeposit: d, stage: "ready", stay: { amountKobo: input.stayAmountKobo, status: "unpaid" }, deposit: { amountKobo: d?.amountKobo ?? 0, status: "unpaid", policyVersion: d?.policyVersion ?? "not-required" }, compensation: { status: "not_required" } }); this.#journeys.set(input.offerId, journey); return journey;
  }
