...
test("AC1: Request to Book, Review request, Submit, Accept and checkout actions have role=button with a name", async () => {
  ...
  try {
    ...
    await c.tab.clickButton("View apartment", "Luxury 2-Bedroom Apartment in Old Ikoyi");
...
test("AC4: Receipts render as markers, not assistant turns, live and after reload", async () => {
  const c = await startContext(375, 812);
  const markers = "[...document.querySelectorAll('#transcript .receipt-marker')].map((node) => node.textContent)";
  const assistantReceipts = "[...document.querySelectorAll('#transcript .turn.assistant')].filter((node) => node.textContent.includes('Draft created')).length";
  try {
    await sendPrompt(c.tab, PROMPT);
    await c.tab.waitForText("Luxury 2-Bedroom Apartment");
    ...
    await c.tab.waitForText("Draft created");
    assert.deepEqual(await c.tab.evaluate<string[]>(markers), ["Draft created"]);
    assert.equal(await c.tab.evaluate<number>(assistantReceipts), 0, "a receipt is never rendered as an assistant turn");

    await c.tab.navigate(`${c.base}/?threadId=${c.threadId}`);
    await c.tab.waitForText("Draft created");
    assert.deepEqual(await c.tab.evaluate<string[]>(markers), ["Draft created"], "the restored timeline keeps the receipt as a marker");
    assert.equal(await c.tab.evaluate<number>(assistantReceipts), 0);
  } finally { await c.close(); }
});
...
async function completeJourney(context: MobileContext, recoverFromPendingPayment = false): Promise<void> {
  ...
  assert.equal(await tab.clickButton("View apartment", "Luxury 2-Bedroom Apartment in Old Ikoyi"), true);
  ...
  await tab.waitForText("Continue to Refundable Security Deposit");
  await capturePhase4(context, "deposit-payment-required");
  assert.equal(await tab.clickButton("Continue to Refundable Security Deposit"), true);
  await tab.waitForText("Check payment status");
