...
test("AC3 — An unverified Guest can submit an otherwise valid Booking Request", async () => { const j = await completeUnverifiedJourney(); try { assert.deepEqual(j.stages[4]!.receipts, ["Booking Request sent"]); } finally { close(j); } });
...
test("AC6 — Conditional Booking Offer acceptance does not require Guest Identity Verification", async () => { const j = await completeUnverifiedJourney(); try { assert.deepEqual(j.stages[5]!.receipts, ["Offer accepted"]); } finally { close(j); } });
...
test("AC9 — A valid Reservation can be created without Guest Identity Verification", async () => { const j = await completeUnverifiedJourney(); try { assert.deepEqual(j.final.receipts, ["Payment verified"]); } finally { close(j); } });
...
test("AC24 — Existing complete Guest journey still passes without a verified identity fixture", async () => { const j = await completeUnverifiedJourney(); try { assert.deepEqual(j.final.receipts, ["Payment verified"]); } finally { close(j); } });
