...
test("AC6 — Delivery failure, Operator decline, and Operator timeout remain distinct", async () => { let now = new Date("2026-09-03T10:00:00Z"); const e = new LocalGuestEnvironment({ databasePath: `.scratch/local-guest/telemetry-outcomes-${Date.now()}.sqlite`, autoDeliverRequests: false, clock: () => now }); try { const a = new LocalGuestApp(e); const t = `g-${crypto.randomUUID()}`; let r = await a.handleTurn(t, "I need an apartment in Ikoyi from 10 Sept for 3 nights for 2 people"); for (let i = 0; i < 4; i++) { assert.equal(r.ok, true); if (!r.ok) return; r = a.handleEvent(t, action(r.surfaces[0]!)); } assert.equal(r.ok, true); now = new Date("2026-09-03T10:06:00Z"); a.getState(t); assert.ok(names(e).includes("booking_request.delivery_failed")); } finally { close(e); } });
