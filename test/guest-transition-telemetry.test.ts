...
test("AC4 — Verification blocking emits a safe reason without identity evidence", async () => { const e = new LocalGuestEnvironment({ databasePath: `.scratch/local-guest/telemetry-verification-${Date.now()}.sqlite`, guestIdentityVerified: false }); try { const a = new LocalGuestApp(e); const t = `g-${crypto.randomUUID()}`; let r = await a.handleTurn(t, "I need an apartment in Ikoyi for 3 nights for 2 people"); for (let i = 0; i < 3; i++) { assert.equal(r.ok, true); if (!r.ok) return; r = a.handleEvent(t, action(r.surfaces[0]!)); } assert.equal(r.ok, true); assert.ok(names(e).includes("request_draft.blocked")); assert.match(JSON.stringify(e.telemetry.events()), /VERIFICATION_REQUIRED/); assert.doesNotMatch(JSON.stringify(e.telemetry.events()), /passport|NIN|BVN|identity evidence/i); } finally { close(e); } });
