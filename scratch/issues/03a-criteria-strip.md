...
- Review follow-up (26 Sept 2026, user decision): below 48rem the strip is a one-line compact summary of the filled criteria with an "Edit search" toggle (`#criteria-toggle`, `aria-expanded`). The chips, editor and undo open under it, and a completed change folds it back. It is at most 70px tall at 375px and 320px, where it had been 105–157px. At 48rem and wider the chips are always shown. Covered by `test/guest-criteria-strip-chromium.test.ts`.
