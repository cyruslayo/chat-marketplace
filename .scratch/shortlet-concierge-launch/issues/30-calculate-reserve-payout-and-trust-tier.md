...
- **Mandatory Committed Accounting Repository & Release Authority**: Settlement calculation strictly requires an authoritative `ProductionRevenueReleaseRecord`, an authoritative `RevenueAccountingRepository`, and mandatory reliability and enforcement authorities. Projections are never produced for uncommitted or free-floating objects; missing any source fails closed. The committed repository record serves as the sole source of truth for financial and payout fields, preventing caller-supplied tampering.
- **Reconciliation from CURRENT Accumulated Ledger State & Offset Accounting**: `ReservePayoutManager` derives current balances across `operator_payable`, `rolling_reserve`, `post_stay_deferred`, `risk_restricted`, and `operator_costs_and_offsets` by folding all committed journals for the release. The consistency invariant accurately accounts for initial Release booking deductions (processor costs, withholding, refunds/credits, offsets) captured in the base Release journal versus post-Release offset changes, guaranteeing `currentSettlementTotal + postReleaseOffsetDelta === authoritativeOperatorNet`.
